...
function noteLayout(f,text){
 let size=10.5,lines=wrap(f,text,size,W-130);
 if(lines.length>12){size=9.7;lines=wrap(f,text,size,W-130)}
 if(lines.length>20){size=9;lines=wrap(f,text,size,W-130)}
 return{size,lines,lineH:size*1.35}
}
...
function drawCenteredBlock(p,lines,start,size,font,color,lineH,minY=62){
 let y=start,i=0;
 for(;i<lines.length;i++){
  if(y<minY)break;
  const ln=lines[i];
  if(ln){const tw=font.widthOfTextAtSize(ln,size);p.drawText(ln,{x:(W-tw)/2,y,size,font,color})}
  y-=lineH;
 }
 return{used:i,y}
}
async function drawProposalImage(o,p,im,x,rowTop,boxW,boxH,f){
 const imgBottom=rowTop-boxH;
 try{
  const j=await o.embedJpg(data(im.dataUrl)),sc=Math.min(boxW/j.width,boxH/j.height),ww=j.width*sc,hh=j.height*sc;
  p.drawRectangle({x,y:imgBottom,width:boxW,height:boxH,borderColor:lineColor,borderWidth:.6,color:rgb(1,1,1)});
  p.drawImage(j,{x:x+(boxW-ww)/2,y:imgBottom+(boxH-hh)/2,width:ww,height:hh})
 }catch(e){console.warn('Immagine proposta non leggibile',e)}
 const caption=String(im.descrizione||'').trim();
 let captionLines=0;
 if(caption){const fit=fittedLines(f,caption,9.5,7.8,boxW,4);let cy=imgBottom-14;captionLines=fit.lines.length;for(const ln of fit.lines){p.drawText(ln,{x,y:cy,size:fit.size,font:f,color:dark});cy-=11}}
 return imgBottom-(captionLines?14+captionLines*11:0)
}
async function proposal(o,m,f,b){
 ...
 const note=String(m.prezzoTesto||'').trim();
 const il=intro?introLayout(f,intro):{size:10.5,lines:[],lineH:14.5};
 const nl=note?noteLayout(b,note):{size:10.5,lines:[],lineH:14.2};
 ...
 for(let pageIndex=0;pageIndex<chunks.length;pageIndex++){
  ...
  if(introPage){
    currentY=718;
    for(const ln of il.lines){if(ln){const tw=f.widthOfTextAtSize(ln,il.size);p.drawText(ln,{x:(W-tw)/2,y:currentY,size:il.size,font:f,color:dark})}currentY-=il.lineH}
    currentY-=18;
  }
  if(chunk.length){
   const margin=55,gap=18,cellW=(W-margin*2-gap)/2;
   const rows=Math.ceil(chunk.length/2);
   for(let row=0;row<rows;row++){
    const rowItems=chunk.slice(row*2,row*2+2),single=rowItems.length===1;
    const onlyOne=chunk.length===1;
    const boxW=single?(onlyOne?Math.min(W-120,430):Math.min(W-155,365)):cellW;
    let boxH=single?(onlyOne?260:195):145;
    const remainingRows=rows-row;
    const minNeededBelow=(remainingRows-1)*(single?190:190)+70;
    const maxAllowed=Math.max(120,currentY-minNeededBelow-70);
    boxH=Math.min(boxH,maxAllowed);
    let rowBottom=Infinity;
    for(let col=0;col<rowItems.length;col++){
      const im=rowItems[col],x=single?(W-boxW)/2:margin+col*(cellW+gap);
      const bottom=await drawProposalImage(o,p,im,x,currentY,boxW,boxH,f);
      rowBottom=Math.min(rowBottom,bottom)
    }
    currentY=rowBottom-20;
   }
  }
  const isLastChunk=pageIndex===chunks.length-1;
  if(isLastChunk&&notePos<nl.lines.length){
    const startY=Math.min(currentY-4,images.length?currentY-4:Math.max(690,currentY));
    const r=drawCenteredBlock(p,nl.lines.slice(notePos),startY,nl.size,b,sage,nl.lineH,58);
    notePos+=r.used;
  }
 }
 while(notePos<nl.lines.length){
   ...
   center(p,(m.sezione||'LA NOSTRA PROPOSTA PER VOI').toUpperCase(),H-35,13,b);
   if(m.titolo)center(p,m.titolo.toUpperCase(),750,15,b);
   const r=drawCenteredBlock(p,nl.lines.slice(notePos),705,nl.size,b,sage,nl.lineH,62);
   if(!r.used)break;
   notePos+=r.used
 }
}
