...
const ERASE={
13:[[127,324.8,505,344.5],[127,617,505,666]],
15:[[92,305.5,500,324.8],[145,334.6,500,353.6],[145,363.1,500,382.1],[92,398.8,505,417.2],[92,425.8,505,444.3]],
16:[[92,371.4,505,414.8],[112,590.7,505,637]],
17:[[131,99,505,118],[128,486,515,504.7],[128,597.1,505,615.8]],
21:[[92,94,240,112.5]]
};
...
S.appendContractV10=async(out,src,raw,p,t)=>{
  ...
  const fonts=await loadFonts(out),italic=fonts.italic,regular=fonts.regular,pdfjs=window.pdfjsLib;
  pdfjs.GlobalWorkerOptions.workerSrc=PDFJS_WORKER;
  const renderDoc=await pdfjs.getDocument({data:raw}).promise,pages={};
  try{
    for(let n=13;n<=21;n++){
      if(ERASE[n])pages[n]=await rasterCleanPage(out,renderDoc,n,ERASE[n]);
      else{const[pg]=await out.copyPages(src,[n-1]);out.addPage(pg);pages[n]=pg}
    }
  ...
  wrapped(pages[13],[127,617,505,666],[631.04,661.05],`fissate per il giorno ${date} presso ${cer} e il ricevimento ${rec}.`,italic,12,9.7,2);
...
async function rasterCleanPage(out,renderDoc,pageNum,boxes){
  const sp=await renderDoc.getPage(pageNum),vp=sp.getViewport({scale:RASTER_SCALE}),c=document.createElement('canvas');
  c.width=Math.ceil(vp.width);c.height=Math.ceil(vp.height);
  const x=c.getContext('2d',{alpha:false,willReadFrequently:true});x.fillStyle='#fff';x.fillRect(0,0,c.width,c.height);
  await sp.render({canvasContext:x,viewport:vp,background:'rgb(255,255,255)'}).promise;
  erase(x,c,boxes);
  const png=await out.embedPng(bytes(c.toDataURL('image/png'))),pg=out.addPage([REF_W,REF_H]);
  pg.drawImage(png,{x:0,y:0,width:REF_W,height:REF_H});c.width=1;c.height=1;try{sp.cleanup()}catch{}return pg;
}
function erase(ctx,c,boxes){
  const sx=c.width/REF_W,sy=c.height/REF_H,img=ctx.getImageData(0,0,c.width,c.height);
  for(const b of boxes)inpaint(img.data,c.width,c.height,b,sx,sy);
  ctx.putImageData(img,0,0);
}
function inpaint(data,w,h,b,sx,sy){
  const pad=2.2,x0=Math.max(1,Math.floor((b[0]-pad)*sx)),y0=Math.max(1,Math.floor((b[1]-pad)*sy)),x1=Math.min(w-1,Math.ceil((b[2]+pad)*sx)),y1=Math.min(h-1,Math.ceil((b[3]+pad)*sy)),rw=x1-x0,rh=y1-y0;
  if(rw<=2||rh<=2)return;
  let m=new Uint8Array(rw*rh);
  for(let y=0;y<rh;y++){let p=((y0+y)*w+x0)*4;for(let x=0;x<rw;x++,p+=4){const lum=.2126*data[p]+.7152*data[p+1]+.0722*data[p+2];if(lum<205)m[y*rw+x]=1}}
  m=dilate(m,rw,rh,Math.max(1,Math.round(1.1*Math.max(sx,sy))));
  let active=[];for(let i=0;i<m.length;i++)if(m[i])active.push(i);
  for(let iter=0;active.length&&iter<40;iter++){
    const next=[];let changed=0;
    for(const li of active){
      if(!m[li])continue;const y=Math.floor(li/rw),x=li-y*rw;let sr=0,sg=0,sb=0,n=0;
      for(let dy=-1;dy<=1;dy++)for(let dx=-1;dx<=1;dx++){if(!dx&&!dy)continue;const nx=x+dx,ny=y+dy;if(nx<0||ny<0||nx>=rw||ny>=rh)continue;const ni=ny*rw+nx;if(m[ni])continue;const q=((y0+ny)*w+x0+nx)*4;sr+=data[q];sg+=data[q+1];sb+=data[q+2];n++}
      if(n){const q=((y0+y)*w+x0+x)*4;data[q]=Math.round(sr/n);data[q+1]=Math.round(sg/n);data[q+2]=Math.round(sb/n);data[q+3]=255;m[li]=0;changed++}else next.push(li);
    }
    if(!changed)break;active=next;
  }
}
...
async function loadFonts(out){
  try{if(!window.fontkit)throw new Error();out.registerFontkit(window.fontkit);const[a,b]=await Promise.all([fetch(FONT_ITALIC).then(ok),fetch(FONT_REGULAR).then(ok)]);return{italic:await out.embedFont(new Uint8Array(a),{subset:true}),regular:await out.embedFont(new Uint8Array(b),{subset:true})}}
  catch(e){return{italic:await out.embedFont(StandardFonts.HelveticaOblique),regular:await out.embedFont(StandardFonts.Helvetica)}}
}
